...
cost(k)     "cost of tour k"
revenue(k)  "revenue of tour k"
;

Binary Variables
x(k, i, j)  "1 if Route from site i to site j in tour k is taken else 0"
y(k, i)     "1 if Attraction site i is visited in tour k else 0"
;
Positive Variable
u(k, i)     "Auxiliary variable for subtour elimination (MTZ method)"
;
...
Equations
    ...
    famous_attraction_constraint    "Famous attraction that can be visited more than once"
    non_famous_constraint           "Non-Famous attraction that can be visited atmost once"
    cost_trip(k)                    "Cost incurred by the trip"
    revenue_trip(k)                 "Revenue of the trip"
    ;

objective..
    z =e= a * sum((k,i), rating(i) * y(k,i)) - (sum((k,i,j), distance(i,j) * fuelCost * x(k,i,j)) + sum((k,i,j), time(i,j)/60 * driverSalary * x(k,i,j)) + sum((k,i),visitTime(i)/60 * driverSalary * y(k,i)) + sum((k,i), entryFee(i) * 9 * y(k,i)));


time_constraint(k).. sum((i, j), time(i, j) * x(k, i, j)) + sum(i, visitTime(i) * y(k, i)) =l= maxTime;

flow_conservation(k,j)$(ord(j) <> 21).. sum(i$(not sameas(i,j)), x(k,i,j)) =e= sum(i$(not sameas(i,j)), x(k,j,i));

dependency_constraint(k, i).. y(k, i) =e= sum(j$(not sameas(i, j)), x(k, i, j));

start_at_edi(k).. sum(j, x(k, '21', j)) =e= 1;

end_at_edi(k).. sum(i, x(k, i, '21')) =e= 1;

subtour_elimination(k, i, j)$(ord(i) <> ord(j) and ord(i) <> 21 and ord(j) <> 21).. u(k, i) - u(k, j) + N * x(k, i, j) =l= N - 1;

cost_trip(k).. cost(k) =e= sum((i,j), distance(i,j) * fuelCost * x(k,i,j)) + sum((i,j), time(i,j)/60 * driverSalary * x(k,i,j)) + sum((i),visitTime(i)/60 * driverSalary * y(k,i)) + sum((i), entryFee(i) * y(k,i));

revenue_trip(k).. revenue(k) =e= a * sum((i), rating(i) * y(k,i))

Set famous_attractions /1, 2, 3, 4, 13/;
Set non_famous_attractions /5, 6, 7, 8, 9, 10, 11, 12, 14, 15, 16, 17, 18, 19, 20/;

famous_attraction_constraint(i)$(famous_attractions(i)).. sum(k, y(k, i)) =g= 1;
non_famous_constraint(i)$(non_famous_attractions(i)).. sum(k, y(k, i)) =l= 1;



Model vrp_max_attract /objective, cost_trip, revenue_trip, time_constraint, flow_conservation, dependency_constraint, start_at_edi, end_at_edi, subtour_elimination, famous_attraction_constraint, non_famous_constraint/;
Solve vrp_max_attract using mip maximizing z;
